...
1. **Initialize the Governance Plane:**
   ...
   ```bash
   kubectl apply -f phase-1/day-07/manifests/01-namespace.yaml
   kubectl apply -f phase-1/day-07/manifests/02-resourcequota.yaml
   kubectl apply -f phase-1/day-07/manifests/03-limitrange.yaml
   ```

2. **Apply Configurations and Secrets:**

   ```bash
   kubectl apply -f phase-1/day-07/manifests/04-configmap.yaml
   kubectl apply -f phase-1/day-07/manifests/05-secret.yaml
   ```

3. **Deploy the Database Tier:**

   ```bash
   kubectl apply -f phase-1/day-07/manifests/06-redis.yaml
   ```
...
4. **Deploy the Frontend Web Tier:**
   ...
   ```bash
   kubectl apply -f phase-1/day-07/manifests/07-web-app.yaml
   ```
