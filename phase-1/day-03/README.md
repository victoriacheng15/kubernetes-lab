...
This day covers Kubernetes workload controllers and the rollout mechanics used to update them safely.
...
Node-level utilities
  DaemonSet
    ├─ Daemon Pod on Node A
    └─ Daemon Pod on Node B

Stateful workloads
  StatefulSet
    ├─ Stateful Pod 0 (bound to PV 0)
    └─ Stateful Pod 1 (bound to PV 1)
```
...
1. **Deploy the Stateless Application:**
   ...
   ```bash
   kubectl apply -f phase-1/day-03/manifests/01-deployment.yaml
   ```
...
2. **Deploy the Node Daemon:**
   ...
   ```bash
   kubectl apply -f phase-1/day-03/manifests/02-daemonset.yaml
   ```
...
3. **Deploy the Stateful Workload:**
   ...
   ```bash
   kubectl apply -f phase-1/day-03/manifests/03-statefulset.yaml
   ```
...
6. **Clean Up:**
   ...
   ```bash
   kubectl delete -f phase-1/day-03/manifests/
   ```
