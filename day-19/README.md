...
1. **Create the Namespace:**
   ...
   ```bash
   kubectl apply -f day-19/manifests/01-namespace.yaml
   ```

2. **Deploy the OOM Workload:**

   ```bash
   kubectl apply -f day-19/manifests/02-oom-pod.yaml
   ```
...
4. **Deploy the Config Error Workload:**
   ...
   ```bash
   kubectl apply -f day-19/manifests/03-config-error-pod.yaml
   ```
