...
```mermaid
flowchart TD
    Start([Pod Created]) --> CheckReqLim{Do all containers define requests and limits?}
    
    CheckReqLim -->|Yes| CheckEqual{Are requests and limits equal for ALL containers?}
    CheckReqLim -->|No| CheckAnyReq{Do any containers define requests or limits?}
    
    CheckEqual -->|Yes| Guaranteed[QoS: Guaranteed\nEviction Priority: Lowest]
    CheckEqual -->|No| Burstable[QoS: Burstable\nEviction Priority: Medium]
    
    CheckAnyReq -->|Yes| Burstable
    CheckAnyReq -->|No| BestEffort[QoS: BestEffort\nEviction Priority: Highest]
```
...
## Lab: Resource Governance and QoS Eviction
...
1.  **Create the Namespace:**
    ...
    ```bash
    kubectl apply -f day-08/manifests/01-namespace.yaml
    ```

2.  **Deploy and Inspect the Guaranteed Pod:**
    Apply the Guaranteed Pod manifest:
    ```bash
    kubectl apply -f day-08/manifests/02-pod-guaranteed.yaml
    ```
...
3.  **Deploy and Inspect the Burstable Pod:**
    ...
    ```bash
    kubectl apply -f day-08/manifests/03-pod-burstable.yaml
    ```
    Verify the Pod is running and check its assigned QoS class:
    ```bash
...
4.  **Deploy and Inspect the BestEffort Pod:**
    ...
    ```bash
    kubectl apply -f day-08/manifests/04-pod-besteffort.yaml
    ```
    Verify the Pod is running and check its assigned QoS class:
    ```bash
...
5.  **Simulate and Observe Memory Limit Enforcement (OOMKilled):**
    ...
    ```bash
    kubectl apply -f day-08/manifests/05-pod-oom.yaml
    ```
