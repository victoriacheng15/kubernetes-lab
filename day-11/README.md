...
## Lab: Static and Dynamic Storage Lifecycle
...
1.  **Create the Namespace:**
    ...
    ```bash
    kubectl apply -f day-11/manifests/01-namespace.yaml
    ```
...
2.  **Deploy and Bind Static Storage:**
    ...
    ```bash
    kubectl apply -f day-11/manifests/02-pv-static.yaml
    ```
    Next, apply the PersistentVolumeClaim which requests a matching `1Gi` volume:
    ```bash
    kubectl apply -f day-11/manifests/03-pvc-static.yaml
    ```
...
3.  **Mount Storage and Verify Persistence:**
    ...
    ```bash
    kubectl apply -f day-11/manifests/04-pod-static.yaml
    ```
...
4.  **Request Dynamic Storage Allocation:**
    ...
    ```bash
    kubectl apply -f day-11/manifests/05-pvc-dynamic.yaml
    ```
    ...
    *Expected Outcome:* The status should show `Bound` (or `Pending` if you are on a custom cluster that requires a Pod to trigger provisioning).
    ...
    ```bash
    kubectl apply -f day-11/manifests/06-pod-dynamic.yaml
    ```
