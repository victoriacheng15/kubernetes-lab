...
## HTTP Routing: Ingress and Ingress Controllers
...
## Lab: Path-Based HTTP Routing via Ingress
...
1.  **Create the Namespace:**
    ...
    ```bash
    kubectl apply -f day-13/manifests/01-namespace.yaml
    ```
...
2.  **Deploy the Backend Web Applications:**
    ...
    ```bash
    kubectl apply -f day-13/manifests/02-deploy-web-apple.yaml
    kubectl apply -f day-13/manifests/03-deploy-web-banana.yaml
    ```
...
3.  **Deploy the Ingress Resource:**
    ...
    ```bash
    kubectl apply -f day-13/manifests/04-ingress-routing.yaml
    ```
