...
## Pod Lifecycle, Restarts, and Init Containers
...
```mermaid
flowchart TD
    Scheduled[Pod Scheduled] --> Pending[Phase: Pending]
    Pending --> Init[Execute Init Containers]
    Init -->|Failure & RestartPolicy Always/OnFailure| Init
    Init -->|Failure & RestartPolicy Never| FailedPhase[Phase: Failed]
    Init -->|Success| AppStart[Start App Containers]
    AppStart --> RunningPhase[Phase: Running]
    RunningPhase -->|Container Crashes & RestartPolicy Always/OnFailure| AppStart
    RunningPhase -->|Container Finishes Success & RestartPolicy Never| SucceededPhase[Phase: Succeeded]
    RunningPhase -->|Container Crashes & RestartPolicy Never| FailedPhase
```
...
## Lab: Inspecting Pod Lifecycles and Init Containers
...
1. **Deploy a basic application Pod:**
   ...
   ```bash
   kubectl apply -f day-02/manifests/01-pod-lifecycle.yaml
   ```
...
3. **Deploy a Pod with an Init Container:**
   ...
   ```bash
   kubectl apply -f day-02/manifests/02-init-container.yaml
   ```
...
4. **Observe Restart Policies in Action:**
   ...
   ```bash
   kubectl apply -f day-02/manifests/03-restart-policy.yaml
   ```
...
5. **Clean Up:**
   ...
   ```bash
   kubectl delete -f day-02/manifests/
   ```
